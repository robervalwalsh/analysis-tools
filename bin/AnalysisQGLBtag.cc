...
int SetTreeBranches(TTree * t) {
  ...
  t->SetBranchAddress("Jet_qgl", jet_qgl_);
  return 0;
}
...
void fill_histogram(TTree* tree, TH2F* plot1, TH2F* plot2) {
  ULong64_t how_many;
  if (debug) {
    how_many = 100;
  } else {
  ...
  for (ULong64_t i = 0; i < how_many; i++) {
    ...
    cout << "Filling FIRST" << endl;
    plot1->Fill(btag_deep_[0], jet_qgl_[0]);
    ...
    UInt_t f = bjet_indexes[0];
    cout << "Filling second plot" << endl;
    plot2->Fill(btag_deep_[f], jet_qgl_[f]);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  // x is btag, y is qgl
  TH2F before("before", "B-tag vs QGL before", 10, 0, 1, 10, 0, 1);
  TH2F after("after", "B-tag vs QGL after", 10, 0, 1, 10, 0, 1);
  
  fill_histogram(tree, &before, &after);
  output.cd();
  before.SetLineColor(kGreen);
