...
int main(int argc, char * argv[])
{
   ...
   analysis.addTree<JetTag> ("Jets","MssmHbbTrigger/Events/hltCombinedSecondaryVertexBJetTagsCalo");
   ...
   for ( int i = 0 ; i < analysis.size() ; ++i )
   {
      ...
      // Jets
      auto jets = analysis.collection<JetTag>("Jets");
      for ( int j = 0 ; j < jets->size() ; ++j )
      {
         JetTag jet = jets->at(j);
         std::cout << "    Jet #" << j << ": ";
         std::cout << "pT  = "     << jet.pt()      << ", ";
