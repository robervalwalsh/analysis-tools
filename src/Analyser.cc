...
Analyser::Analyser(int argc, char * argv[])
{
   ...
   analysis_ = new Analysis(inputlist_);
   
   // Jets
   if ( jetsCol_ != "" )       analysis_->addTree<Jet> ("Jets",jetsCol_);
   // Trigger path info
   if ( triggerCol_ != "" )    analysis_->triggerResults(triggerCol_);
   // Trigger objects
   if ( triggerObjects_.size() > 0 && triggerObjDir_ != "" )
   {
      for ( auto & obj : triggerObjects_ )
      {
         analysis_->addTree<TriggerObject> (obj,Form("%s/%s",triggerObjDir_.c_str(),obj.c_str()));
      }
   }
   // JSON for data   
   if( !isMC_ && json_ != "" ) analysis_->processJsonFile(json_);
   
   // output file
   if ( outputRoot_ != "" )
   {
      std::string sr = "SR";
      if ( ! signalregion_ ) sr = "CR";
      boost::algorithm::replace_last(outputRoot_, ".root", "_"+sr+".root"); 
      hout_= std::shared_ptr<TFile>(new TFile(outputRoot_.c_str(),"recreate"));
   }
}
...
bool Analyser::event(const int & i)
{
   ...
   if ( ! analysis_->triggerResult(hltPath_) ) return false;
